...
public class BeatTableServiceImpl implements BeatTableService {
  ...
  private Pair increasePair(
    ...
    return new Pair(
        pair.getWinning() + winIncrement, pair.getLosing() + loseIncrement
    );
  }

  private List<String> collectChoices(final List<CastVote> castVotes) {
    return castVotes.stream().map(CastVote::getPreferences)
        .flatMap(List::stream)
        .map(RankedChoice::getChoiceId).collect(Collectors.toList());
  }
