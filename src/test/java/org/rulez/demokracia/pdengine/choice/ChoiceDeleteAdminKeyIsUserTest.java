...
public class ChoiceDeleteAdminKeyIsUserTest extends ChoiceTestBase {
  ...
  @Test
  public void for_unmodifiable_vote_users_cannot_delete_choices() {
    final Choice choiceToDelete = createChoice(USER, false);
    assertThrows(
  ...
  public void
    ...
    choiceService.deleteChoice(
        voteData.adminInfoWithNoNeededAssurances, choiceToDelete.getId()
    );

    assertThrows(
        () -> choiceService.getChoice(
            voteData.voteWithNoNeededAssurances.getId(), choiceToDelete.getId()
        )
    )
  ...
  public void deleteChoice_saves_vote_if_the_choice_is_deleted() {
    ...
    verify(voteService).saveVote(voteData.voteWithNoNeededAssurances);
  }
  ...
  private Choice
    ...
    voteData.voteWithNoNeededAssurances.getParameters()
        .setAddinable(isAddinable);
    final Choice choiceToDelete = new Choice(CHOICE1, userName);
    voteData.voteWithNoNeededAssurances.addChoice(choiceToDelete);
